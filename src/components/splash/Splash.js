...
const Splash = () => {
  ...
  return (
    ...
    <div className="all-doctors">
      ...
      <p className="text-muted text-center notice-word">{dotString.repeat(doctors.length)}</p>
      <Slider {...settings}>
        {doctors.length > 0 && doctors.map((doctor) => (
          <div key={doctor.id} className="doctor">
            <Link to="/login" className="doctor-image">
              <img src={doctor.image} alt="doctor" className="card-img-top" />
            </Link>
            <div className="doctor-info">
              <Link to="/login" className="doctor-name">
                {doctor.name}
              </Link>
